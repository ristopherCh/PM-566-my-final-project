...
There is good reason to expect that the devastation wrought by COVID-19 would lead to increasing deaths by overdose. Substance abuse is associated with unemployment or underemployment, lack of career opportunities, social isolation, mental health issues, and homelessness. As COVID-19 has undeniably contributed to each of these factors, an investigation into whether the pandemic has driven the increase in overdose deaths is warranted.
...
gct <- ggplot(covid_usa, aes(x = date, y = new_case)) +
  geom_line() +
  labs(title = "New US COVID-19 cases", subtitle = "Source: data.cdc.gov") +
...
\begin{footnotesize}
Figure 2: Cumulative COVID-19 cases in a representative sample of US states.
\end{footnotesize}
...
\begin{footnotesize}
Figure 3: Cumulative COVID-19 associated deaths in a representative sample of US states.
\end{footnotesize}
...
gdu <- ggplot(drugs_usa, aes(x = date, y = deaths, color = indicator)) +
  ...
  annotate(x = as.Date("2019-12-01"), y = 70000, label = "Start of COVID-19", geom="text", angle = 90, size = 4) +
...
od_cov2 <- ggplot(covid_od, aes(x = case_perc, y = odpop_perc, label = state_name)) +
  geom_point(aes(color = region)) +
  geom_smooth(span = 1) +
  labs(title = "Cases of COVID vs deaths by overdose in US states", x = "COVID-19 cases, by % of state pop", y = "% of total state population killed by OD, April 2020 - April 2021", subtitle = "Source: data.cdc.gov API")  +
  scale_x_continuous(breaks = scales::pretty_breaks(n = 8)) +
  scale_y_continuous(breaks = scales::pretty_breaks(n = 8))
```
...
\begin{footnotesize}
Figure 6: COVID-19 deaths up until April 2021 by percent of state population plotted against the percent increase in yearly deaths by overdose from April 2020 to April 2021, per state. The trend has high variance and is slightly negatively correlated. There are small clusters of regional similarity, but no trends to draw any conclusions from.
\end{footnotesize}
...
```{r}
ggod_cov2 <- ggplotly(od_cov2) %>% layout(title = "Cases of COVID vs deaths by overdose in US states", font = list(size = 26, family = "Times New Roman"), xaxis = list(title = "COVID-19 cases, by % of state pop", titlefont = list(size = 22), tickfont = list(size = 18)), yaxis = list(title = "% of Total State Population Killed by OD, 04/2020 - 04/2021", titlefont = list(size = 22), tickfont = list(size = 18)))
...
\begin{footnotesize}
Figure 7: COVID-19 cases up until April 2021 by percent of state population plotted against the percent of total state population killed by overdose. The trend has high variance and is slightly negatively correlated.
\end{footnotesize}
...
\begin{footnotesize}
Figure 9: Regional trends in deaths due to overdose (percent increase from the year ending in April 2020 to the year ending in April 2021).
\end{footnotesize}
...
unem <- ggplot(unemployment_plot, aes(x = date, y = value, color = STATE)) +
  ...
  scale_x_date(breaks = seq(as.Date("2018-01-01"), as.Date("2021-10-01"), by="3 months"), date_labels = "%m/%y")
```
...
\begin{footnotesize}
Figure 10: The unemployment rate in a representative sample of US states, showing the effect the economic shutdown had on employment.
\end{footnotesize}

\vspace{5pt}

To examine the possibility that the unemployment experienced as a result of the COVID-19 lockdowns led to increased drug usage and deaths by overdose, below is a visualization of the relationship between these factors.
...
u3 <- ggplot(covid_od_unemp, aes(x = unemp_change, y = odincrease_apr20apr21, label = state_name)) +
  geom_point() +
  geom_smooth(span = 1) +
  labs(title = "Increase in Unemployment vs % Increase in Yearly OD Deaths", x = "Relative Increase in Unemployment %, April 2020 - October 2020", y = "% Increase in Deaths by OD, 04/2019-04/2020 to 04/2020-04/2021") + 
  scale_x_continuous(breaks = scales::pretty_breaks(n = 8)) +
  scale_y_continuous(breaks = scales::pretty_breaks(n = 8))
```
...
\begin{footnotesize}
Figure 12: Unemployment percentage increase above pre COVID-19 levels vs raw deaths due to overdose. The relationship appears positively correlated, but with greater variance than the prior plot. No obvious correlation with geographical region is evident.
\end{footnotesize}
...
\begin{footnotesize}
Figure 13: Unemployment percentage increase above pre COVID-19 levels vs percent increase in drug overdoses per capita in the year coinciding with the COVID-19 pandemic.
\end{footnotesize}
...
Figure 13 is the most accurate portrayal of the real relationship between COVID-19 unemployment and drug overdose deaths: The *relative increase in the percent of the total state population that died due to drug overdose* from the 12 months ending in April 2020 to the 12 months ending in April 2021, compared to the *increase in the percent of the state population experiencing unemployment* from February 2020, before the pandemic, to October 2020, when unemployment began to recede in most states. A positive correlation is suggested, but not certain.
...
ggecon <- ggplot(econplotdf_real, aes(x = TimePeriod, y = DataValue, color = GeoName)) +
  ...
  scale_x_date(breaks = seq(as.Date("2018-01-01"), as.Date("2021-06-01"), by="3 months"), date_labels = "%m/%y")
...
To adjust for the magnitude of difference between the size of each state's economy, GDP values are normalized to pre-pandemic levels:
...
Finally, to examine whether the degree of economic slowdown suffered in each US state correlates with deaths due to drug overdose, this data is plotted together:
...
w1 <- ggplot(covid_od_econ, aes(x = econdrop, y = odincrease_apr20apr21, label = state_name)) +
  geom_point() +
  geom_smooth(span = 1) +
...
# Methods
Data for the population of the US and its States was collected from the census.gov’s API portal. It was converted to a data.table by the methods instructed in lecture, and associated with other data.tables using the data.table merge method.
...
Data for drug overdoses in the USA was also accessed through the data.cdc.gov API. It was merged with the census data to be able to calculate overdose percentages by state population. Later, it was merged with the COVID-19 data to investigate the relationship between overdoses and COVID-19 infection rates.
Data for unemployment was downloaded from the Bureau of Labor Statistics website.
Data for GDP was received from the Bureau of Economic Analysis API.
The data was very reliable, as it was thoroughly gathered by the CDC and other reliable governmental agencies, and needed very little cleaning or wrangling. Data exploration was mainly done visually using ggplot2 line and scatterplots.
